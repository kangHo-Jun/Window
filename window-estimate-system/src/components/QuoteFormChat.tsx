...
import type { ChatQuoteData, QuoteCompleteHandler } from '@/types/quote';
...
export default function QuoteFormChat({ onComplete }: { onComplete: QuoteCompleteHandler }) {
  ...
  const createMessage = (text: string, sender: Message['sender']): Message => {
    ...
    return { id: messageIdRef.current, text, sender };
  };

  useEffect(() => {
    const timer = window.setTimeout(() => {
      setIsTyping(false);
      setMessages([createMessage(CHAT_QUESTIONS[0].text, 'ai')]);
    }, 800);
  ...
  const handleOptionClick = (option: string) => {
    const newMessages: Message[] = [...messages, createMessage(option, 'user')];
    ...
    if (nextStep < CHAT_QUESTIONS.length) {
      ...
      window.setTimeout(() => {
        setIsTyping(false);
        setMessages((prev) => [...prev, createMessage(CHAT_QUESTIONS[nextStep].text, 'ai')]);
      }, 800);
      return;
    }
    ...
    window.setTimeout(() => {
      ...
      setMessages((prev) => [
        ...prev,
        createMessage("감사합니다! 입력하신 정보를 바탕으로 가장 알맞은 창호를 제안해 드릴게요. (결과는 콘솔에 출력되었습니다)", 'ai')
      ]);

      const resultJson: ChatQuoteData = {
        type: "chat",
        data: {
          space: newAnswers.Q1,
          count: newAnswers.Q2,

...
## 7. Conversational RAG UX 설계

### 7.1 핵심 패러다임

> 소비자는 "수퍼로이", "열관류율" 같은 용어를 모른다.
> 지인이가 소비자의 언어(상황/감각)로 질문을 만들고,
> 클릭만으로 심층 대화로 이끈다.

기존: 소비자가 질문 → 지인이가 답변 (수동적)
변경: 지인이가 질문을 만들어 → 소비자가 클릭 (능동적)

### 7.2 설계 원칙 3가지

| 원칙 | 내용 |
|---|---|
| 용어 없음 | 소비자 언어(상황/감각)로만 버튼 생성 |
| 클릭으로만 | 타이핑 최소화, 버튼이 대화를 이끔 |
| 깊이 자동화 | 클릭할수록 전문 지식으로 자연 진입 |

### 7.3 진입 버튼 설계 (용어 없는 상황 언어)

```
지인이: "창호 교체를 생각하시는 이유가 뭔가요?"

[겨울에 너무 추워요]        ← 단열 RAG flow
[소음이 심해요]             ← 기밀/차음 RAG flow
[결로/곰팡이가 생겨요]      ← 기밀/단열 RAG flow  
[햇빛이 너무 강해요]        ← 차폐 RAG flow
[그냥 오래돼서 바꾸려고요]  ← 견적 flow 직행
```

### 7.4 단열 클릭 시 전체 Flow 시뮬레이션

**1단계 — 공감 + 원인 1줄**
```
지인이: "창가에 앉으면 냉기가 느껴지시는 거죠?
         대부분 창호 단열 등급이 낮아서
         열이 새는 경우예요."
...
**2단계 — "고치면 실제로 따뜻해지나요?" 클릭**
```
지인이: "네, 확실히 달라져요 🙂
         3~4등급 창호를 1등급으로 바꾸면
         창가 온도가 3~5도 올라가는 경우가 많아요."
...
**3단계 — "난방비도 줄어드나요?" 클릭**
```
지인이: "30평 기준으로 연 30~50만원
         절감되는 사례가 많아요.
         교체 비용을 3~4년이면 회수하는 셈이에요 💡"
...
**4단계 — "어떤 제품으로 바꿔야 하나요?" 클릭**
```
지인이: "단열에 가장 효과적인 건
         이중창 + 아르곤 가스 충진 조합이에요.
         LX지인 F-250I 기준 열관류율 0.66으로
         국내 최고 1등급이에요 🏆"
...
```typescript
{
  ...
  "related_questions": [
    "상황/감각 언어 질문 1",
    "상황/감각 언어 질문 2",
    "견적 유도 질문 3"  // 마지막은 항상 견적 연결
  ],
  "trigger_quote": false  // true면 견적 Flow 진입
}
```

**AIChatBot.tsx 버튼 렌더링**
```typescript
...
**RAG Flow → 견적 Flow 전환 조건**
```typescript
if (response.trigger_quote === true) {
  // 견적 Flow 진입
  setMode("quote")
  sendMessage("견적 시작")
}
...
### 7.7 불편사항별 RAG Flow 매핑

| 소비자 버튼 | 내부 RAG 카테고리 | 최종 견적 유도 |
|---|---|---|
| 겨울에 너무 추워요 | energy_spec | 이중창 + 아르곤 추천 |
| 소음이 심해요 | product_spec (기밀) | 시스템창 추천 |
| 결로/곰팡이가 생겨요 | energy_spec + install | 단열 + 시공 품질 |
| 햇빛이 너무 강해요 | energy_spec (차폐) | 수퍼더블로이 추천 |
| 오래돼서 바꾸려고요 | general | 견적 Flow 직행 |

---

## 8. 연구 기반 UX 설계 전략

> 출처: RAG_기반_프로액티브_대화_시스템_연구.md (2026.04.30)
> 학술 근거: ProMISe(EACL 2024), CHI 2024, Perplexity AI 사례 분석
...
| 전략 | 근거 데이터 | 적용 우선순위 |
|---|---|---|
| 연관 질문 버튼 동적 생성 | 전환율 2.8% → 14.2% (5배) | 1순위 |
| 입력 완료 직후 개입 타이밍 | 신뢰도 최고 (CHI 2024) | 1순위 |
| BANT Hot Lead 감지 | 상담 예약 42% 증가 | 2순위 |
| Zero-step 예약 자동 입력 | 완결률 2.5배 향상 | 2순위 |

---

### 8.2 ProMISe 기반 연관 질문 동적 생성 (1순위)

**근거:** EACL 2024 ProMISe 연구  
사용자가 무엇을 물어야 할지 모를 때,  
시스템이 RAG 기반으로 연관 질문을 동적 생성하여 제공.
...
**버튼 렌더링 규칙:**
- 전문 용어 사용 금지 (소비자 상황/감각 언어 사용)
- 최대 3개
- 마지막 버튼은 항상 견적 또는 상담 연결

**예시:**
```
지인이: "단열 성능 차이가 확실히 있어요.
         겨울 창가 온도가 3~5도 달라집니다."

[실제로 얼마나 따뜻해지나요?]
[난방비는 얼마나 줄어요?]
[우리집에 맞는 제품 보고 싶어요]  ← 견적 진입
```

---

### 8.3 CHI 2024 타이밍 전략 — "검증으로서의 개입" (1순위)

**근거:** CHI 2024 Proactive Dialogue 연구  
시스템 개입 타이밍:
- Before (먼저 제안): 신뢰도 낮음
- Synchronous (동시): 간섭으로 인식
- **After (완료 직후): 신뢰도 최고 ← 적용**

**구현 규칙:**  
사용자가 정보 입력을 완료한 직후,  
칭찬 또는 확인 멘트 1줄 → 다음 단계 진행.
...
### 8.4 BANT 기반 Hot Lead 감지 (2순위)

**근거:** Guided Selling 산업 사례 (상담 예약 42% 증가)

대화 중 아래 4가지를 자동 감지하여  
Hot Lead 점수를 산출하고 상담 버튼 노출 시점 결정.

| 항목 | 감지 방법 | 점수 |
|---|---|---|
| Budget (예산) | "얼마나 들어요?" 질문 시 | +2 |
| Authority (결정권) | "남편/아내와 상의" 미언급 | +1 |
| Need (필요성) | 불편사항 2개 이상 선택 | +2 |
| Timeline (시기) | "즉시" 또는 "1~3개월" 선택 | +2 |

```typescript
// consumerGrouping.ts 확장
interface BANTScore {
  budget: number      // 0~2
...
// 총점 6점 이상 → Hot Lead → 즉시 방문 상담 예약 버튼 노출
// 총점 3~5점 → Warm Lead → 견적 완료 후 상담 버튼 노출
// 총점 3점 미만 → Cold Lead → PDF 다운로드 유도
```

---

### 8.5 Zero-step 예약 자동 입력 (2순위)

**근거:** 완결률 2.5배 향상 데이터

상담 신청 버튼 클릭 시,  
대화에서 수집된 정보가 자동으로 입력된 상태로 폼 표시.

```typescript
// 자동 입력 항목
{
  name: fields.customerName,
  pyeong: fields.pyeong,
  housing_type: fields.housingType,
  problems: fields.problems,
  timeline: fields.timeline,
  estimated_price: quoteResult.lx
}
// → Google Sheets / Firestore consultations 자동 저장
```
...
| 순위 | 항목 | 구현 위치 | 예상 효과 |
|---|---|---|---|
| 1 | 연관 질문 버튼 동적 생성 | `route.ts` + `AIChatBot.tsx` | 전환율 5배 |
| 1 | 입력 완료 직후 검증 멘트 | `route.ts` 프롬프트 | 신뢰도 상승 |
| 2 | BANT Hot Lead 감지 | `consumerGrouping.ts` 확장 | 예약 42% 증가 |
| 2 | Zero-step 예약 자동 입력 | `AIChatBot.tsx` + Sheets | 완결률 2.5배 |
